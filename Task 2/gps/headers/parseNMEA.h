...
namespace NMEA
{
  ...
  /* A REGular EXpression containg the standard format of a NMEA sentance,
   * used in isWellFormedSentence seen below, and covers the sentence contents
   * described below
   */
  const std::regex sentenceFormatting("\\$GP[A-Z]{3},[^\\$\\*]*\\*[0-9A-Fa-f]{2}");
  ...
  /* Constant Map to hold the indexes of the longitude, longitude direction,
   * latitude, latitude direction and elevatiion for each format.
   * Used in interpretSentenceData()
   */
   const std::map<std::string, std::vector<int>> dataIndexByFormat = {
    // {"Format", {latitude, latitude direction, longitude, longitude direction, elevation}}
       {"GLL", {0, 1, 2, 3}},
       {"RMC", {2, 3, 4, 5}},
       {"GGA", {1, 2, 3, 4, 8}}
   };
